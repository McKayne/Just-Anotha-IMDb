...
class BookmarkedDataSource: NSObject, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if SecondViewController.movies.count == 0 {
            tableView.isHidden = true
        } else {
            tableView.isHidden = false
        }
        return SecondViewController.movies.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = UITableViewCell(style: UITableViewCellStyle.default, reuseIdentifier: "bookmarkedCell")
        cell.textLabel?.textAlignment = .center
        
        let poster = UIImageView(frame: CGRect(x: 0, y: 0, width: 100, height: 100));
        poster.image = SecondViewController.movies[indexPath.row].poster
        cell.contentView.addSubview(poster)
        
        let title = UILabel(frame: CGRect(x: 100, y: 0, width: 220, height: 30))
        title.text = SecondViewController.movies[indexPath.row].title
        cell.contentView.addSubview(title)
        
        let plot = UILabel(frame: CGRect(x: 100, y: 30, width: 220, height: 70))
        plot.textColor = .gray
        plot.numberOfLines = 3
        plot.text = SecondViewController.movies[indexPath.row].plot
        plot.sizeToFit()
